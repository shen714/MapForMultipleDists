...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		//Map<Integer, Integer> verticAndDegreeMap = new TreeMap<Integer, Integer>(valueComparator.reversed());
		List<Entry<Integer, Integer>> degreeEntrySequence = new ArrayList<Entry<Integer, Integer>>();
		List<Integer> degreeSequence = new ArrayList<Integer>();
		
		for (int i = 0; i < numVertices; i++) {
			int degree = this.getNeighbors(i).size() + this.getInNeighbors(i).size();
			SimpleEntry<Integer, Integer> verticAndDegree = new SimpleEntry<Integer, Integer>(i, degree);
			degreeEntrySequence.add(verticAndDegree);
		}
		
		degreeEntrySequence.sort(valueComparator.reversed());
		for(int i = 0; i < degreeEntrySequence.size();i++) {
			degreeSequence.add(degreeEntrySequence.get(i).getValue()); 
		}
		
		/*for(Integer vertic : verticAndDegreeMap.keySet() ) {
			degreeSequence.add(vertic);
		}*/
	...
	public static void main (String[] args) {
		...
		List<Integer> vertexDistance2 = graphFromFile.getDistance2(3);
		for(int i = 0; i < vertexDistance2.size();i++) {
			System.out.println(vertexDistance2.get(i));
		}
